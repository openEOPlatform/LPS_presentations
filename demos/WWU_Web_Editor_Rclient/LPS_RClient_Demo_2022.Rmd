...
Retreiving the pixels values directly through openEO is also possible via `aggregate_spatial()`.

## View COGs

First we replace the monthly aggregation to a yearly aggregation.

```{r change_process_node, eval=FALSE}

# how to do this in the best way?

```

then we register our process graph as a job on a backend. A job is created on the backend and executed asynchronously upon request.
...
And finally, view the cog interatively. Currently using a custom function, a generic COG viewing will be available soon via `mapview`(<https://github.com/r-spatial/mapview/issues/400>)

```{r mapview_cog}
source("/home/pzellner@eurac.edu/git_projects/LPS_presentations/demos/WWU_Web_Editor_Rclient/view_cog.R")
view_cog(url = url, min_scale = 0, max_scale = 0.4)
